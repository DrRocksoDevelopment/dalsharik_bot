...
Правила:
- Пиши ТОЛЬКО по фактам из контекста: распределение голосов, правильный ответ, объяснение, источники.
...
- Приведи зрителей к верному варианту и объясни, почему он правильный.
...
export function buildHostPrompt(ctx: HostContext, instruction: string = DEFAULT_HOST_PROMPT): string {
  ...
  if (question.sources.length > 0) parts.push(`Источники: ${question.sources.join(', ')}`);
  parts.push('');
  ...
  if (ctx.nextEventLocalTime) {
    parts.push(`Следующее событие: в ${ctx.nextEventLocalTime} по местному времени`);
  }
