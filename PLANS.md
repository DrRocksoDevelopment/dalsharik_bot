...
Операционное планирование ведётся в GitHub:

- Доска: https://github.com/orgs/DrRocksoDevelopment/projects/5 — статусы `Backlog` → `Ready` → `In Progress` → `In Review` → `Done`, приоритеты `P0`–`P2`.
- Каждая фича — issue (крупные — эпик с sub-issues), милстоуны по релизам (`v0.3.0`, `v0.4.0`, ...).
- Лейблы совпадают с префиксами коммитов (AGENTS.md): `feat`, `bug`, `perf`, `docs`, `chore`, плюс `epic` и `backlog` (перспектива).
- Правило: операционный статус живёт в GitHub (доска/issues), здесь — только техника: решения, архитектура, порядок работ. Статусы ниже синхронизируются по факту релиза.

Бэклог фич: [EPIC] Рейтинг чатов (#1), [EPIC] Сезоны (#2), Шаринг результата — перспектива (#11), Хот-тейк перед ответом (#12), [EPIC] Недельный дайджест (#13), [EPIC] Достижения (#14).

Данные для будущих фич уже хранятся: `AnswerRecord` (isCorrect, answeredAt, questionId), `UserProfile` (score, bestStreak, correct/wrong), `StoredQuestionMetrics` (category, difficulty, answer_distribution). Ограничения: `bestStreak` — суммарный за всё время (стрик за период пересчитывается из ответов); скан `answers.json` — O(N) (см. долг по SQLite/ротации).

### Решения по фичам

- Сезоны: чаты, добавленные в середине сезона, засчитываются в рейтинг с момента появления (не со следующего сезона). (#10, закрыто 12.08.2026)
